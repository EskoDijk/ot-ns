...
traffic group <group-id> <node-id> [<node-id> ...]
```

The parameters are similar to the [ping](ping) command. To use `udp mcast`, first one or more multicast groups need to 
be created using `traffic group`. Concurrent traffic can be generated by issueing multiple `traffic` commands.
