...
Generate unicast and/or multicast traffic between nodes.

```shell
traffic udp <src-id> <dst-id> [<addr-type>] [datasize <sz>] [count <cnt>] [interval <intval>] [hoplimit <hoplim>]
traffic udp mcast <src-id> <dst-group-id> [datasize <sz>] [count <cnt>] [interval <intval>]
traffic group <group-id> <node-id(s)>
```

Most parameters are similar to the [ping](ping) command. To use `udp mcast`, first one or more multicast groups need to 
be created using `traffic group`. Concurrent traffic can be generated by issuing multiple `traffic` commands.

To select node IDs for the traffic group creation, individual nodes, or ranges, or a combination, or "all" can be 
used, as shown in more detail in the [del](#del) command.

Traffic types other than `udp` like tcp, tls, coap, or coaps are currently not implemented.
